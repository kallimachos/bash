...
# networking
sed -i 's/IPADDR.*/IPADDR=10.1.11.21/' /etc/sysconfig/network-scripts/ifcfg-eth0
echo "GATEWAY=10.1.11.1" >> /etc/sysconfig/network-scripts/ifcfg-eth0
sed -i 's/IPADDR.*/IPADDR=10.1.12.31/' /etc/sysconfig/network-scripts/ifcfg-eth1
sed -i 's/IPADDR.*/IPADDR=10.1.10.31/' /etc/sysconfig/network-scripts/ifcfg-eth2
...
while [[ "$repeat" = "true" ]]; do
    retries+=1
    echo "Try number $retries..."
    ssh compute 'bash -s ~/vxlan.sh' && repeat="false"
sleep 5
done
